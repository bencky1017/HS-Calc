...
var calc=(function calc(){
	...
	if (curlvl<20&destlvl<20&curlvl<destlvl|destlvl==''|destnum=='') {
		// 当前不为空，目标有空 合成算法
		if (curlvl!=''&curnum!=''&(destlvl==''|destnum=='')) {
			// console.clear();
			var temp_lvl=curlvl;
			var temp_num=curnum;
		...
		if (destlvl!=''&destnum!='') {
			...
			while(temp_dlvl>=stoplvl){
				...
				for (var i = 0; i < extlen; i++) {
					if (temp_dlvl==ext_lvl[i]) {
						temp_ext=ext_num[i]==''?0:ext_num[i];
					}
				}
				result_td='<tr clsass="res_tr"><td>'+temp_dlvl+'</td><td>'+temp_dnum+'</td><td>0</td><td>'+temp_ext+'</td></tr>';
				$('.result').append(result_td);
				temp_dlvl--;
				temp_dnum=(division(temp_dnum-temp_ext).s);
			}
		}
	}else{
		$('.result tr').remove();
...
var focusclick=(function focusclick(){
	$('input[type=search]').on('focus keydown',function(e){
		// 你按了键盘enter
